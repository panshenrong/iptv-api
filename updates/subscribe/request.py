...
async def get_channels_by_subscribe_urls(
    ...
    def process_subscribe_channels(subscribe_info: str | dict) -> defaultdict:
        ...
        try:
            response = None
            try:
                if subscribe_url.startswith('https://gyssi.link'):
                    subscribe_url=subscribe_url+"?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOjI0NDUxOTY1NjY5MjEwMjEsImlhdCI6MTc0NTEwMzMzOCwiZXhwIjoxNzc3OTM1MzM4fQ.BpuPhEQHdVTGBswzVLZTVX5oUJO_gsGu68a39FnK2M4"
                response = (
                    retry_func(
                        lambda: session.get(
